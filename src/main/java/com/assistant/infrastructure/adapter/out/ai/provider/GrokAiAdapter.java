/**
 * Objetivo: Consumir proveedor Grok/Groq usando endpoint OpenAI-compatible.
...
@Slf4j
@Service
public class GrokAiAdapter implements com.assistant.application.service.AiProviderService {
    ...
    public GrokAiAdapter(
        ...
        AiProvidersProperties.ProviderProperties config = aiProvidersProperties.getGrok();
    ...
    public String generateResponse(String systemContext, String userPrompt, String model) {
        try {
            ...
            if (response == null || response.isBlank()) {
                throw new IllegalStateException("GROK_EMPTY_RESPONSE");
            }
            return response;

        } catch (IllegalStateException e) {
            log.error("Error de autenticación o configuración en Grok: {}", e.getMessage());
            String code = e.getMessage() != null ? e.getMessage().split(":")[0] : "GROK_AUTH_FAILED";
            throw new IllegalStateException(code, e);
        } catch (Exception e) {
            log.error("Error al llamar a Grok", e);
    ...
    public AiToolResponse generateResponseWithTools(String systemContext, String userPrompt, String model) {
        AiToolResponse directToolCall = AiToolRoutingSupport.detectDirectToolCall(userPrompt, "Grok", log);
        ...
        return toolResponseParser.parse(content, "Grok");
    }
}
